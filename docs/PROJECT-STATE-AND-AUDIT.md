...
11. ⏸️ **`payment_method='bonus'` + defer >24ч** списывает баланс, а не бонусный пул.
    **2026-07-10 находка: фича «бонус/бесплатный час» ДОРМАНТНА** — за всю историю
    **0 броней бонусом**, из 98 выданных бонусов 0 списанных (все `active`). Авто-
    применение бонуса не реализовано НИГДЕ (ни в `pricing`, ни в create-пути) — не только
    в defer. Т.е. это не surgical-фикс, а полноценная фича (интеграция в прайсинг + оба
    пути списания + возврат при отмене + UI). Владелец решил семантику (бонус+остаток
    деньгами; при отмене возвращать), но **отложено до реального запуска бонусов** —
    вкладывать риск в денежную бухгалтерию ради 0-использования нецелесообразно.
    **ОТЛОЖЕНО (спящая фича).**
