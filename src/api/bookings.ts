...
export const bookingsApi = {
    ...
    /**
     * Разделить бронь на несколько подряд идущих частей — например, двухчасовой
     * слот на две сессии по часу, чтобы к каждой привязать своего клиента.
     *
     * `parts` — длительности в минутах по порядку; их сумма обязана совпадать с
     * длительностью брони. Деньги не двигаются: сумма частей всегда равна
     * исходной цене (смежные часы движок и так считает одной цепочкой).
     *
     * Первой частью остаётся исходная бронь — её id, привязанная CRM-сессия и
     * событие календаря сохраняются. Возвращает все части по порядку.
     */
    splitBooking: async (bookingId: string, parts: number[]): Promise<BookingHistoryItem[]> => {
        const response = await api.patch<any[]>(`/bookings/${bookingId}/split`, { parts });
        return (response.data || []).map(mapToFrontend);
    },
